...
4. `workpacks/instances/workpack-manager_core-architecture_02/00_request.md`
...
- **Workpack folder naming**: standalone `<slug>` or grouped `<group-id>_<slug>_<NN>` pattern from protocol. Groups contain a `group.meta.json` with the execution DAG.
